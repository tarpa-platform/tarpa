...
const selectedPermissions = ref(props.permissions)
const form = useForm({
    ...
    <div class="mt-8 flow-root overflow-hidden">
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <TableFullwidth id="resource-permissions">
                <template #thead>
                    ...
                    <template v-for="permission in permissions">
                        <TableHeadCol class="capitalize text-center">{{ permission.name }}</TableHeadCol>
                    </template>
                </template>
                <tr v-for="resource in resources" :key="resource.key">
                    ...
                    <template v-for="permission in permissions">
                        <TableRowCol visible="relative whitespace-nowrap">
                            <template #cell>
                                <span class="sr-only">New role can {{ permission.name }} {{ resource.name }}.</span>
                                <input type="checkbox" class="-mt-2 mx-auto block h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600" :value="resource.key + '.' + permission.key" v-model="selectedPermissions" />
                            </template>
                        </TableRowCol>
                    </template>
                </tr>
            ...
            <div class="mt-8 flex items-center gap-4">
                <PrimaryButton @click.prevent="form.transform((data) => ({...data, permissions: selectedPermissions})).patch(route('role.update', role), { preserveScroll: true })">
                Save
                </PrimaryButton>
